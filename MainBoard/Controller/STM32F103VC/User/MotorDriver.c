...
void MotorDriver_Init(uint8_t nMotorCount)  //初始化电机驱动，nMotor=1，初始化电机A，nMotor=2，初始化电机A和B，以此类推，从1到4
{
  ...
  TIM_TimeBaseInit(TIM1, &TIM_TimeBaseStructure);
	...
	switch (nMotorCount)
	{
		case 4:  //初始化电机D，IN1-PD8，IN2-PA8（TIM1-1），EF-PD9
		...
		case 3:   //初始化电机C，IN1-PC8，IN2-PA9（TIM1-2），EF-PC9
		...
		case 2:		//初始化电机B，IN1-PD10，IN2-PA10（TIM1-3），EF-PD11
				RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOD, ENABLE);
		...
		case 1:		//初始化电机A，IN1-PD14，IN2-PA11（TIM1-4），EF-PD15
				RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOD, ENABLE);
...
void Encoder_Init(uint8_t nEncoderCount)
{
	...
	{
		case 4:  //初始化编码器D，A-PD12(TIM4-1)，B-PD13（TIM4-2）；需要remap
				RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOD, ENABLE);
		...
		case 3:   //初始化编码器C，A-PC6(TIM8-1)，B-PC7（TIM8-2）
			  ...
			  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_6|GPIO_Pin_7;
				...
				TIM8->CNT = ENC_TIM_ARR/2;
				MotorDirver_Tim8_Update_Count = 0;
				TIM_Cmd(TIM8, ENABLE);			
		
		
		case 2:		//初始化编码器B，A-PA15(TIM2-1)，B-PB3（TIM2-2）；需要remap
				RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA, ENABLE);
			  ...
			  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_15;
				...
				TIM2->CNT = ENC_TIM_ARR/2;
				MotorDirver_Tim2_Update_Count = 0;
				TIM_Cmd(TIM2, ENABLE);

		case 1:		//初始化编码器A，A-PB4(TIM3-1)，B-PB5（TIM3-2）；需要remap
				RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB, ENABLE);
